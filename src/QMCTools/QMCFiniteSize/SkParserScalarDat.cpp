...
void SkParserScalarDat::read_sk_file(const string& fname)
{
  ...
  while(!f.eof())
  {
    ...
    if (obsname.find("--------") != string::npos) break;
...
//        app_log()<<"  rho_i_"<<rho_i.size()<<" = "<<val_tmp<<" +/- "<<err_tmp<<endl;
  ...
  //      app_log()<<"  rho_r_"<<rho_r.size()<<" = "<<val_tmp<<" +/- "<<err_tmp<<endl;
      ...
      else APP_ABORT("ERROR SkParserScalarDat.  Unexpected rhok_e_X... found.  ");
      obsname = ""; // reset obsname to avoid double-registering an entry
    }
    else
      continue;
