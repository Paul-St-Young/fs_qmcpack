...
#include "Numerics/GridUtilities.h"

namespace qmcplusplus
{

SkParserBase::SkParserBase(): isParseSuccess(false),
...
{
  if(!isParseSuccess) APP_ABORT("SkParserBase::compute_grid(..) : Initial parse failed");
...
{
//  cout<<"In get_grid(..)\n"; 
  if(!isGridComputed) compute_grid();
...
//  cout<<"In compute_sk()\n";
  ...
  //app_log()<<" Hiyo.  Going to parse the kgrids now.\n";
  ...
  for(IndexType i=0; i<kgridraw.size(); i++)
  {
    ...
    newindex=nx*Ny*Nz+ny*Nz+nz;
//    app_log()<<kgridraw[i][0]<<" "<<kgridraw[i][1]<<" "<<kgridraw[i][2]<<" "<<nx<<" "<<ny<<" "<<nz<<endl;
